                ...
                <div class="table-responsive px-2 px-md-4 pb-4">
                    @if($filter === 'bulanan' || $filter === 'tahunan')
                        @php
                            ...
                            // Sinkronkan filter kegiatan dengan kolom export: hanya kegiatan yang kolomnya dipilih di export_fields yang tampil di rekap
                            // Jika export_fields diisi, hanya tampilkan kegiatan yang memiliki data pada kolom export_fields
                            if(request('export_fields')) {
                                // Asumsi: semua kegiatan tetap tampil, tapi kolom per kegiatan hanya yang ada di export_fields
                                // Jika ingin filter kegiatan juga, bisa tambahkan filter_kegiatan
                                if(request('filter_kegiatan')) {
                                    $uniqueKegiatan = $uniqueKegiatan->whereIn('id', (array)request('filter_kegiatan'));
                                }
                            } else if(request('filter_kegiatan')) {
                                $uniqueKegiatan = $uniqueKegiatan->whereIn('id', (array)request('filter_kegiatan'));
                            }
