        ...
        if ($filter === 'bulanan') {
            ...
            foreach ($absensi as $a) {
                ...
                if ($kegiatanObj && $kegiatanObj->jenis_kegiatan == 'sholat_jamaah') {
                    ...
                    // Pastikan hanya status hadir yang dihitung untuk keterlambatan sholat
                    if ($a->is_late && $a->status === 'hadir') {
                        $rekapBulanan[$a->mahasantri_id][$a->kegiatan_id]['terlambat_sholat']++;
                    }
                }
                // Untuk pengajian, hanya status hadir yang dihitung untuk keterlambatan
                if (
                    isset($rekapBulanan[$a->mahasantri_id][$a->kegiatan_id]) &&
                    $a->is_late &&
                    $a->status === 'hadir' &&
                    $kegiatanObj &&
                    $kegiatanObj->jenis_kegiatan === 'pengajian'
                ) {
