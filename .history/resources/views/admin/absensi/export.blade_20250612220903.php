...
<table border="1">
    <thead>
        <tr>
            <th colspan="{{ 2 + count($kegiatan)*count($exportFields) }}" style="text-align:center; font-weight:bold; font-size:16px;">Rekap Absensi {{ ucfirst($filter) }} - {{ $namaPeriode }}</th>
        </tr>
        <tr>
            <th rowspan="2">NIM</th>
            <th rowspan="2">Nama Mahasantri</th>
            @foreach($kegiatan as $k)
                <th colspan="{{ count($exportFields) }}" style="text-align:center">
